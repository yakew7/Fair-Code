...
| | Who | Role |
|:--|-----|------|
| <a href="https://github.com/yakew7"><img src="https://github.com/yakew7.png" width="48" height="48" alt="yakew7"></a> | **Yash Kewlani** - [@yakew7](https://github.com/yakew7) | Creator and maintainer. Author of the seven audits, the `faircode` library and benchmark harness, the Open Dataset Profiler, the website, and the bulk of the 61 explainers. Code owner for `faircode/`, `paper/`, every `audit.yaml`, and project policy (`CLAUDE.md`, `CONTRIBUTING.md`); co-owner of `explainers/`. |
...
**43 merged PRs · 60 commits · first merged 2026-07-06**

Code owner for [`/Makefile`](Makefile) and [`/scripts/`](scripts/) - the newest core-ownership grant,
narrower in scope than the other three core contributors' broader areas, added after the highest
merged-PR count of any contributor besides the maintainer and a real, accepted write-access
invitation (a CODEOWNERS entry for a read-only collaborator is silently skipped by GitHub's own
review routing).

The [Predictive Parity](explainers/predictive-parity.md) explainer ([#72](https://github.com/yakew7/Fair-Code/pull/72)) - the ProPublica vs. Northpointe dispute as two correct fairness checks that cannot both hold. Then the contributor tooling: [`Makefile`](Makefile), [`.pre-commit-config.yaml`](.pre-commit-config.yaml), and the local-setup section of [CONTRIBUTING.md](CONTRIBUTING.md) ([#125](https://github.com/yakew7/Fair-Code/pull/125), closing issue #114). Later refactored the first-interaction workflow to find a contributor's genuinely earliest issue/PR ([#227](https://github.com/yakew7/Fair-Code/pull/227)), and added proper `<th scope="col">`/`<caption>` markup to `faircode/report.py`'s HTML report tables ([#260](https://github.com/yakew7/Fair-Code/pull/260), closing issue #254). Then test coverage for `generate_favicons.py`'s malformed-`logo.svg` error paths ([#278](https://github.com/yakew7/Fair-Code/pull/278), closing issue #245), and the missing `proxy` extra in `audits.yml`'s profiler job install line ([#311](https://github.com/yakew7/Fair-Code/pull/311), closing issue #298). Then a bundled four-issue PR: `faircode compare` reporting ignored `.xlsx` sheets, the stale "five audits" README wording, the `bug_report.yml` audit dropdown missing two domains, and `CODEOWNERS` missing `profiler.css` ([#314](https://github.com/yakew7/Fair-Code/pull/314), closing issues #300, #308, #307, and #310). Then added Tenant Screening to the `PROJECT_ANCHORS`/`projectAnchors` anchor maps that turn a related-project Markdown link into a homepage anchor, fixing the one audit missing from both ([#357](https://github.com/yakew7/Fair-Code/pull/357), closing issue #350). Then added `.yml`/`.yaml` to `check_em_dash.py`'s scanned extensions and allowlisted the two issue templates that intentionally demonstrate the banned character ([#362](https://github.com/yakew7/Fair-Code/pull/362), closing issue #343). Then three PRs in one day: added `benchmark` to `make setup`'s extras list, which had silently skipped a large chunk of the test suite for anyone following `CONTRIBUTING.md`'s documented setup step ([#480](https://github.com/yakew7/Fair-Code/pull/480), closing issue #473); then two fully-verified explainers, [Treatment Equality](explainers/treatment-equality.md) ([#481](https://github.com/yakew7/Fair-Code/pull/481), closing issue #344) and [The Fairness-Accuracy Trade-off](explainers/fairness-accuracy-tradeoff.md) ([#482](https://github.com/yakew7/Fair-Code/pull/482), closing issue #345), both anchored to a real run of this repo's own benchmark harness rather than invented numbers. Then six more PRs in one day: fixed `faircode/SPEC.md`'s stale prefix-match worked example, broken by an earlier keyword-matching fix ([#505](https://github.com/yakew7/Fair-Code/pull/505), closing issue #489); added a `make audits` target running every audit's `unfair.py`/`fair.py` plus the CLI smoke tests, folded into `make check`, so a local green check can no longer mask a `run-audits` CI failure ([#506](https://github.com/yakew7/Fair-Code/pull/506), closing issue #492); fixed another stale SPEC.md leftover, this time documenting `proxy_hints` parameters a previous fix had already removed ([#507](https://github.com/yakew7/Fair-Code/pull/507), closing issue #499); made `check_generated_files_current.py` rebuild from source via a real `git worktree` instead of only diffing against the last commit ([#508](https://github.com/yakew7/Fair-Code/pull/508), closing issue #502); gave non-numeric age sentinels their own categorical group instead of silently folding them into `missing_pct`, catching and fixing a self-introduced regression against negative-age sentinels before committing ([#509](https://github.com/yakew7/Fair-Code/pull/509), closing issue #487); and closed out that batch by making `TargetSpec.compute()` exclude rows with a missing target label instead of silently scoring them as a negative class ([#510](https://github.com/yakew7/Fair-Code/pull/510), closing issue #488). Then a 14-PR day: `index.html`'s overstated COMPAS dataset size ([#528](https://github.com/yakew7/Fair-Code/pull/528), closing issue #518); `report.py` rendering `"-0.0 pp"` for a tiny negative share change ([#529](https://github.com/yakew7/Fair-Code/pull/529), closing issue #515); a missing "N more groups" notice on the reference-baseline sub-table ([#530](https://github.com/yakew7/Fair-Code/pull/530), closing issue #516); `get_benchmark_results` silently ignoring a filter on a column the chosen `kind` lacks ([#531](https://github.com/yakew7/Fair-Code/pull/531), closing issue #512); `parse_reference`'s percent-vs-fraction scale decision being table-wide instead of per-column ([#532](https://github.com/yakew7/Fair-Code/pull/532), closing issue #513); out-of-range `min_share`/threshold params producing self-contradictory profiler output instead of erroring ([#533](https://github.com/yakew7/Fair-Code/pull/533), closing issue #511); `assets/profiler-engine.js`'s `NA_TOKENS` disagreeing with pandas' real `na_values` ([#534](https://github.com/yakew7/Fair-Code/pull/534), closing issue #491); the web profiler's JSON export recording raw `currentOpts` instead of resolved provenance params ([#535](https://github.com/yakew7/Fair-Code/pull/535), closing issue #490); compare-page cards showing "none drift" for a comparison that was actually skipped ([#536](https://github.com/yakew7/Fair-Code/pull/536), closing issue #519); `_intersections()`'s `labelize()` still dropping non-numeric age sentinels a sibling fix had already fixed for `profile()`'s main breakdown ([#537](https://github.com/yakew7/Fair-Code/pull/537), closing issue #524); `figures.py` blending different protected attributes into one meaningless bar for multi-attribute audits ([#538](https://github.com/yakew7/Fair-Code/pull/538), closing issue #525); `encode_features()` scrambling numeric order for a column with a non-numeric sentinel ([#539](https://github.com/yakew7/Fair-Code/pull/539), closing issue #514); a follow-up regeneration commit after one of the day's earlier doc fixes left generated output stale ([#540](https://github.com/yakew7/Fair-Code/pull/540)); and four new explainers - [Simpson's Paradox](explainers/simpsons-paradox.md), [Differential Privacy](explainers/differential-privacy.md), [Subgroup Fairness](explainers/subgroup-fairness.md), and [Conditional Demographic Parity](explainers/conditional-demographic-parity.md) ([#541](https://github.com/yakew7/Fair-Code/pull/541), closing issues #483-#486) - three of which needed post-merge correction for a reintroduced audit-number mislabel, several rounding slips, and one factually wrong verdict claim in `simpsons-paradox.md`'s own worked example. Most recently, a ten-PR batch: corrected the Benefits Denial README's proxy list to include `fnlwgt` alongside the existing four, joining [@slsgzs-cloud](https://github.com/slsgzs-cloud)'s independently-submitted [#557](https://github.com/yakew7/Fair-Code/pull/557) for the same issues ([#558](https://github.com/yakew7/Fair-Code/pull/558), closing issues #545, #546); fixed two `faircode/manifest.py` validation gaps - NaN rows silently counted in `disadvantaged_mask()`'s returned mask, and an all-empty `RowFilter` silently filtering nothing ([#559](https://github.com/yakew7/Fair-Code/pull/559), closing issues #547, #549); made `significance_report`'s `confidence` parameter actually change the `significant` flag instead of being ignored ([#560](https://github.com/yakew7/Fair-Code/pull/560), closing issue #548); corrected `counterfactual-fairness.md`'s fabricated COMPAS numbers to the repo-canonical figures ([#561](https://github.com/yakew7/Fair-Code/pull/561), closing issue #551); fixed three stale repo-meta references across `llms.txt`, ROADMAP.md's traction table, and `.github/DEAD-FILE-AUDIT.md` ([#562](https://github.com/yakew7/Fair-Code/pull/562), closing issues #554, #555, #556); made `build_explainers.py`'s and `explainers-ui.js`'s table parsers both honor GFM's `\|` escape and brought the JS heading-offset back into parity with Python ([#563](https://github.com/yakew7/Fair-Code/pull/563), closing issues #552, #553); corrected a fabricated ~20.9pp AI-hiring gap repeated across three explainers to the real 21.62%/17.10%/4.51pp figures ([#564](https://github.com/yakew7/Fair-Code/pull/564), closing issue #550); added the [Reject Option Classification](explainers/reject-option-classification.md) explainer, contrasting it against this repo's own S4 `ThresholdOptimizer` ([#565](https://github.com/yakew7/Fair-Code/pull/565), closing issue #542); and fixed two stale Script-Execution-Output blocks pasted from earlier versions of their own detection code - `reject-inference.md`, adding an explicit `RandomForestClassifier` cross-platform-drift caveat in the process ([#567](https://github.com/yakew7/Fair-Code/pull/567), closing issue #522), and `underdiagnosis-bias.md` ([#566](https://github.com/yakew7/Fair-Code/pull/566), closing issue #523). Every PR in this batch was independently re-verified against a fresh run of its own code before being credited here; that review also caught and fixed a regression in [@slsgzs-cloud](https://github.com/slsgzs-cloud)'s #557 (see their entry below) and a recurrence of the #562 explainer-count drift after #565, neither part of any PR.
...
A full-backlog sweep of issues found in review, in one day: fixed `fit_post_processing()` raising a raw `train_test_split` error on a singleton target class ([#448](https://github.com/yakew7/Fair-Code/pull/448), closing issue #446 - the guard only covered `train_test_split`'s own failure mode, and still crashed deeper inside fairlearn for a genuinely-too-sparse minority class; corrected directly, not part of the PR). Then represented a profile with zero detected dimensions as explicitly unmeasured (`overall_score`/`grade: null`, a `dimensions_detected` flag, a `note`) instead of a fabricated `0`/`"F"`, across both engines, the CLI, MCP, and every report format ([#449](https://github.com/yakew7/Fair-Code/pull/449), closing issue #444). Wrapped terminal-screenshot lines longer than 92 characters instead of silently clipping them off-canvas ([#450](https://github.com/yakew7/Fair-Code/pull/450), closing issue #443). Made the CLI sniff CSV/TSV delimiters instead of trusting the extension blindly, matching the web engine's existing behavior ([#451](https://github.com/yakew7/Fair-Code/pull/451), closing issue #439). Fixed the web engine's CSV parser silently stripping quote characters that appear after other field content ([#452](https://github.com/yakew7/Fair-Code/pull/452), closing issue #441), and its delimiter sniffer breaking on a quoted field containing a newline ([#453](https://github.com/yakew7/Fair-Code/pull/453), closing issue #440). Added the missing "…and N more groups" truncation notice to `to_html`'s single-profile report ([#454](https://github.com/yakew7/Fair-Code/pull/454), closing issue #433). Then closed out the two remaining age-handling gaps: negative age sentinels no longer get fabricated into a `75+` group ([#455](https://github.com/yakew7/Fair-Code/pull/455), closing issue #431), and date-vs-age detection now samples the whole column instead of just its first 50 rows ([#456](https://github.com/yakew7/Fair-Code/pull/456), closing issue #432) - though the merge landing #456 alongside #455's own new test spliced the two tests' bodies together incorrectly, silently zeroing out this fix's own parity coverage; fixed directly afterward by reconstructing both test bodies from their original commits.
...
**3 merged PRs · 13 commits · first merged 2026-09-08**

Fixed the MCP `list_explainers` wrapper silently swallowing a `FileNotFoundError` instead of converting it to a `ToolError` the way its `get_explainer`/`get_benchmark_results` siblings already do ([#503](https://github.com/yakew7/Fair-Code/pull/503), closing issue #500). Then bumped `requirements-lock.txt`'s numpy/pandas and other pins up to `requirements.txt`'s own declared minimums, which the lock file had drifted below ([#504](https://github.com/yakew7/Fair-Code/pull/504), closing issue #493). Most recently, independently found and fixed the same three doc issues [@propcgamer20-png](https://github.com/propcgamer20-png) also fixed in [#558](https://github.com/yakew7/Fair-Code/pull/558) - COMPAS's raw-percentage-point vs. relative reduction formula, Benefits Denial's missing `fnlwgt` proxy, and ROADMAP.md's stale traction table ([#557](https://github.com/yakew7/Fair-Code/pull/557), closing issues #545, #546, #555); #557 merged first, with #558's overlapping changes landing a minute later.
