...
### Fixed
- **Negative age sentinels became a fabricated `75+` group** (closes #431, by [@be-student](https://github.com/be-student), [#455](https://github.com/yakew7/Fair-Code/pull/455)) - both profiler engines now reject negative numeric and signed-string ages as missing before banding, skew, and intersection calculations.
- **Date strings appended after the first 50 age rows were fabricated into the `75+` band** (closes #432, by [@be-student](https://github.com/be-student), [#456](https://github.com/yakew7/Fair-Code/pull/456)) - date detection now uses a deterministic sample spread across the whole column in both profiler engines. The merge that landed this alongside #455's own new test spliced the two tests' bodies together wrong, silently dropping this fix's own parity coverage to zero assertions while breaking the other test outright; fixed directly (not part of the PR) by reconstructing both test bodies from each PR's original commit.
- **`report.py`'s `to_html` dropped groups past the display cap with no truncation notice, unlike `to_terminal`/`compare_to_html`** (closes #433, by [@be-student](https://github.com/be-student), [#454](https://github.com/yakew7/Fair-Code/pull/454)) - a shared HTML profile report could look complete while silently hiding up to 8+ groups. Both Python and the web UI now show the same "…and N more groups" notice, and `faircode/SPEC.md` documents the shared 12-group display contract explicitly for the first time.
- **A semicolon-delimited `.csv` file parsed correctly in the web profiler but broke in the CLI** (closes #439, by [@be-student](https://github.com/be-student), [#451](https://github.com/yakew7/Fair-Code/pull/451)) - Python's `read_table` only sniffed the delimiter for an unrecognized extension, trusting `.csv`/`.tsv` blindly; the JS engine always sniffed. Python now sniffs too, falling back to the extension's implied delimiter only when sniffing is ambiguous.
- **JS engine's `parseCSV` stripped quote characters mid-field, diverging from pandas' quoting semantics** (closes #441, by [@be-student](https://github.com/be-student), [#452](https://github.com/yakew7/Fair-Code/pull/452)) - a `"` was treated as significant anywhere in a field; pandas only treats one as significant at the true start of a field. A common "space after delimiter" export style (`Male, "White", 25`) silently disagreed on the parsed group label between engines. Now only enters quote mode when the field buffer is still empty.
- **JS engine's `sniffDelimiter` broke on a quoted field containing a newline, unlike Python's `csv.Sniffer`** (closes #440, by [@be-student](https://github.com/be-student), [#453](https://github.com/yakew7/Fair-Code/pull/453)) - naive line-splitting corrupted the sample whenever a sampled line had an embedded newline inside quotes, falling back to the wrong default delimiter. Row-splitting for sniffing is now quote-aware.
- **Terminal screenshots silently clipped text beyond 92 characters** (closes #443, by [@be-student](https://github.com/be-student), [#450](https://github.com/yakew7/Fair-Code/pull/450)) - `render_terminal_png.py` now wraps each captured line before measuring and drawing the canvas, preserving explicit blank lines and every rendered glyph within the documented width.
