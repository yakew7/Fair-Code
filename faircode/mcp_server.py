...
def _list_explainers_impl(tag=None):
    ...
    summary/tags), optionally filtered to those carrying `tag`. Reads
    faircode/_explainers/data.json, the package-internal generated mirror
    of assets/explainers-data.json (see issue #388 - explainers/ and
    assets/ live outside what pyproject.toml ships, so a real
    `pip install faircode[mcp]` never has the originals on disk). The
    mirror is regenerated by scripts/build_explainers.py, so it's always
    exactly the current published set - never re-derived or cached
    separately."""
